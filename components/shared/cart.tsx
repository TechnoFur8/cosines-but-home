            ...
            {data.cartProduct.length > 0 ?
                <div className={"space-y-6"}>
                    ...
                    <div className={"flex flex-col space-y-3"}>
                        <span className={"text-[#737373] line-through"}>Скидка: {data.cartProduct.reduce((acc, el) => acc + el.discount, 0).toLocaleString("ru-RU", { style: "currency", currency: "RUB", maximumFractionDigits: 0, minimumFractionDigits: 0 })}</span>
                        <span className={"text-sm text-[#737373]"}>Итого: {data.cartProduct.reduce((acc, el) => acc + el.price, 0).toLocaleString("ru-RU", { style: "currency", currency: "RUB", maximumFractionDigits: 0, minimumFractionDigits: 0 })}</span>
                    </div>
                </div>
                :
