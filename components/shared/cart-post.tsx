...
export const CartPost = ({ productId, size }: Props) => {
    ...
    return (
        <Button className={"cursor-pointer"} disabled={isLoading} onClick={() => handleClickCartPost(productId, size)}>{isLoading ? <><LoaderCircle className={"animate-spin"} /> Добовляем в корзину </> : "В корзину"}</Button>
    )
}